...
def process_file(file_path, script_path, file_type):
    try:
        ...
        logger.error(f"Import script failed with code {result.returncode}")
        if result.stdout:
...
def monitor_for_files():
    ...
    logger.info(
        f"Target files: {WORKSTATION_XLS_FILENAME}, {TESTBOARD_XLS_FILENAME}, {SNFN_XLS_FILENAME}"
    )
    logger.info(
        f"Import scripts: {os.path.basename(IMPORT_WORKSTATION_SCRIPT)}, "
        f"{os.path.basename(IMPORT_TESTBOARD_SCRIPT)}, {os.path.basename(IMPORT_SNFN_SCRIPT)}"
    )
    ...
    while True:
        try:
            ...
            if os.path.exists(workstation_xls):
                logger.info(
                    f"Workstation file detected: {WORKSTATION_XLS_FILENAME} at "
                    f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                )
                logger.info("Starting workstation file processing pipeline...")

                success = process_file(workstation_xls, IMPORT_WORKSTATION_SCRIPT, "workstation")

                if success:
                    logger.info("Workstation file processing completed successfully")
                else:
                    logger.error("Workstation file processing failed")
            elif os.path.exists(workstation_xlsx):
                logger.info(
                    f"Workstation file detected: {os.path.basename(workstation_xlsx)} at "
                    f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                )
                logger.info("Starting workstation file processing pipeline...")

                success = process_file(workstation_xlsx, IMPORT_WORKSTATION_SCRIPT, "workstation")
            ...
            if os.path.exists(testboard_xls):
                logger.info(
                    f"Test board file detected: {TESTBOARD_XLS_FILENAME} at "
                    f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                )
                logger.info("Starting test board file processing pipeline...")

                success = process_file(testboard_xls, IMPORT_TESTBOARD_SCRIPT, "testboard")

                if success:
                    logger.info("Test board file processing completed successfully")
                else:
                    logger.error("Test board file processing failed")
            elif os.path.exists(testboard_xlsx):
                logger.info(
                    f"Test board file detected: {os.path.basename(testboard_xlsx)} at "
                    f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                )
                logger.info("Starting test board file processing pipeline...")

                success = process_file(testboard_xlsx, IMPORT_TESTBOARD_SCRIPT, "testboard")
            ...
            if os.path.exists(snfn_xls):
                logger.info(
                    f"SnFn file detected: {SNFN_XLS_FILENAME} at "
                    f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                )
                logger.info("Starting SnFn file processing pipeline...")

                success = process_file(snfn_xls, IMPORT_SNFN_SCRIPT, "snfn")

                if success:
                    logger.info("SnFn file processing completed successfully")
                else:
                    logger.error("SnFn file processing failed")
            elif os.path.exists(snfn_xlsx):
                logger.info(
                    f"SnFn file detected: {os.path.basename(snfn_xlsx)} at "
                    f"{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
                )
                logger.info("Starting SnFn file processing pipeline...")

                success = process_file(snfn_xlsx, IMPORT_SNFN_SCRIPT, "snfn")
